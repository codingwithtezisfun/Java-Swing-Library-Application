...
public class UpdateStudent {
    ...
    public UpdateStudent() {
        ...
        JLabel label = new JLabel("<html><u>UPDATE STUDENT FORM</u></html>");
        label.setFont(new Font("Arial", Font.PLAIN, 25));
        label.setForeground(new Color(186, 135, 89));
        gb.gridx = 0;
        gb.gridy = 0;
        gb.insets = new Insets(5, 1, 5, 1);
        upper.add(label, gb);
        ...
        FetchnameField = new JTextField(25);
        gbc.gridx = 1;
        gbc.gridy = 0;
        ...
        admissionNoField = new JTextField(25);
        gbc.gridx = 1;
        gbc.gridy = 4;
        gbc.fill = GridBagConstraints.HORIZONTAL;  // Ensure horizontal fill
        gbc.gridwidth = 2;
        ...
        courseComboBox = new JTextField(25);
        gbc.gridx = 1;
        gbc.gridy = 7;
        gbc.insets = new Insets(5, 1, 5, 1);
    ...
    private void updateStudent(int studentLibraryId) {
        // Get the updated values from the fields
        String updatedName = nameField.getText();
        String updatedAdmissionNumber = admissionNoField.getText();
        String updatedPhoneNumber = phoneField.getText();
        String updatedEmail = emailField.getText();
        String updatedCourseName = courseComboBox.getText();  // Assuming this field holds the course name

        try (Connection connection = DriverManager.getConnection(url, username, password)) {
            // Your update SQL statement...
            String updateSql = "UPDATE student SET names = ?, admission_number = ?, phone_number = ?, email = ?, course_id = ? WHERE student_library_id = ?";
            try (PreparedStatement updateStatement = connection.prepareStatement(updateSql)) {
                updateStatement.setString(1, updatedName);
                updateStatement.setString(2, updatedAdmissionNumber);
                updateStatement.setString(3, updatedPhoneNumber);
                updateStatement.setString(4, updatedEmail);
                // Set the course_id based on the course name
                updateStatement.setInt(5, getCourseIdByName(updatedCourseName));
                updateStatement.setInt(6, studentLibraryId);
                ...
                if (rowsAffected > 0) {
                    ...
                    JOptionPane.showMessageDialog(null, "Student not found or update failed.", "Update Failed",
    ...
    // Check if the student ID is in use in any of the specified tables
    ...
    private int getCourseIdByName(String courseName) {
        ...
        try (Connection connection = DriverManager.getConnection(url, username, password)) {
            ...
            ex.printStackTrace();
            JOptionPane.showMessageDialog(null, "Error: Could not fetch course ID from the database.", "Error", JOptionPane.ERROR_MESSAGE
            );
        }
